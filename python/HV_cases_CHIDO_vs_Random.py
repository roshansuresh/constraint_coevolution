# -*- coding: utf-8 -*-
"""
Compute Hypervolume stats for all runs comparing CHIDO and Random Search
@author: roshan94
"""
...
problem_dir = 'Truss\\' # Truss, Artery, Assign, Partition, (C1_DTLZ1, Simple DTLZ1_2, UF1 are test problems) 
# (only Truss problem has Random Search runs)
...
case_bools['Random Search'] = [False for i in range(len(heuristic_names) + 1)] # No heuristics enforced
case_bools['CHIDO'] = [True for i in range(len(heuristic_names) + 1)] # All heuristics enforced through Coevolutionary Interior Penalization

plot_colors = {} # black and blue for Random Search & Coev. Penalty respectively
plot_colors['Random Search'] = '#000000'
plot_colors['CHIDO'] = '#56B4E9'
...
for case_key in list(case_bools.keys()):
    ...
    for i in range(n_runs):
        ...
        ## read csv files associated with internal MOEA runs and store collated objectives and constraints
            
        # Read file contents and sort by NFE
        if heurs_dir == 'Random Search\\':
            alg_dir = ''
            fitness_dir = ''
            
            coev_filename = 'random_search_heuristic_weights.csv'
        ...
        current_filepath = results_dir + problem_dir + heurs_dir + int_pop_dir + alg_dir + fitness_dir + 'run ' + str(i) + '\\'
